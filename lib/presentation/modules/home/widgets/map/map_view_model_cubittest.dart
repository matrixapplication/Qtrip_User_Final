...
// import 'package:weltweit/core/utils/alerts.dart';
...
//   static const double _tiltCamera = 50; // for camera using
...
//   initGoogleMap(GoogleMapController? controller) async {_controller =controller; if (currentUserLocation == null) {getCurrentUserLocation();} }
//   Future<void> zoomIn() async { if (mapViewController != null) {await setZoom(mapViewController!, true);}}
//   Future<void> zoomOut() async { if (mapViewController != null) {await setZoom(mapViewController!, false);}}
//
//
//
//
...
//         icon: BitmapDescriptor.fromBytes(await getBytesFromAsset(Assets.imagesCurrentLocationMarker, 100)));
...
//         log(_tag, 'getCurrentUserLocation :: $currentUserLocation');
//
...
//           log(_tag, 'getCurrentUserLocation :: moving moving camera to $currentUserLocation');
...
//       mapControllerStage = MapControllerStage.done;
//       _moveCamera();
//       log(_tag, 'getCurrentUserLocation :: moving moving camera to $initCameraPosition');
//       log(_tag, 'getCurrentUserLocation :: moving moving camera to $currentUserLocation');
//     } catch (e) {
...
//   }
//
//   Future<void> selectDestinationLocationFromMap({required LatLng destinationPosition,required BuildContext context, scaffoldKey}) async {
//     print('desssssssssoooooo $destinationLocation');
//     print('desssssssssoooooo');
//     try {
//       var addressesCountryForCurrentLocation = await geocoding.placemarkFromCoordinates(currentUserLocation?.latitude??0.0, currentUserLocation?.longitude??0.0);
//       var firstForCurrentLocation = addressesCountryForCurrentLocation.first;
//       var addresses = await geocoding.placemarkFromCoordinates(destinationPosition.latitude, destinationPosition.longitude);
...
//             destinationName: addresses.first.street == null || (addresses.first.street??'').isEmpty ? addresses.first.name : addresses.first.street);
...
//       } else {
//
//         Alerts.showSnackBar('خارج النطاق',alertsType: AlertsType.info);
//         // showSnack(
//         //     context: context,
//         //     msg: "خارج النطاق",
//         //     fullHeight: 30.0,
